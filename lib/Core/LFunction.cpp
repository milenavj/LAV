...
LFunction::LFunction(llvm::Function *f, LModule* p) : _Function(f), 
                                          _Parent(p), 
...
void LFunction::AddBlocks()
{
  ...
  for (bbIt = _Function->begin(), bbIe = _Function->end(); bbIt != bbIe; ++bbIt) 
  {
...
static bool isUsedOutsideOfDefiningBlock(llvm::Instruction *I) {
  ...
  for (llvm::Value::use_iterator UI = I->use_begin(), E = I->use_end(); UI != E; ++UI)
      if (llvm::cast<llvm::Instruction>(*UI)->getParent() != BB || llvm::isa<llvm::PHINode>(*UI))
          return true;
  return false;
}
...
{ 
  std::map<llvm::BasicBlock*, LBlock*>::const_iterator iter = _LlvmToF.find(bb);
...
{
  for (llvm::inst_iterator II = inst_begin(_Function), E = inst_end(_Function); II != E; ++II) 
  {
...
//bool LFunction::AddAllocationFixedAddress(llvm::AllocationInst *ai, std::vector<std::string>& variables, std::vector<LVariableInfo>& fvis )
bool LFunction::AddAllocationFixedAddress(llvm::AllocaInst *ai, std::vector<std::string>& variables, std::vector<LVariableInfo>& fvis )
{
  ...
  int size = GetParentModule()->GetTargetData()->getTypeAllocSize(ai->getAllocatedType());

    //FIXME ovo je teska krpljevina
    //treba ove adrese da se dodaju na drugom mestu jer ako nema poziva funkcije onda se one ne dodaju a
    //to je onda pogresno
    //FIXME *1000 je krpljevina da se adrese ne bi gadjale ako neko prekoraci tacno toliko ondaje greska
    ...
    if(_Addresses.find(GetOperandName(ai))==_Addresses.end())
      _Addresses.insert(std::pair<std::string, unsigned>(GetOperandName(ai), size*1000));
    else
...
void LFunction::AddGlobalVariables(std::vector<std::string>& variables, std::vector<LVariableInfo>& fvis )
{
std::set<llvm::GlobalVariable*> GlobalVariables = GetParentModule()->GlobalVariables();
std::set<llvm::GlobalVariable*>::const_iterator i = GlobalVariables.begin(), ie = GlobalVariables.end();
std::set<llvm::GlobalVariable*>& GlobalReferences = GetParentModule()->GlobalReferences();
...
void LFunction::AddLocalVariables(std::vector<std::string>& variables, std::vector<LVariableInfo>& fvis )
{
...
//          else isUsed = isUsedOutsideOfDefiningBlock(&*II);
       ...
       if (!isUsed)
       {
//        if(llvm::AllocationInst *ai = llvm::dyn_cast<llvm::AllocationInst>(&*II))
        ...
        {    
          ...
          if (llvm::AllocaInst *ai = llvm::dyn_cast<llvm::AllocaInst>(&*II))
          {
            ...
            //std::cerr << "\n\n\n\n\n\n\n\n\nnije allocation a uzima se referenca ?!?" << GetOperandName(&*II) << std::endl;
            continue;
            }           
...
void LFunction::AddFunctionArguments(std::vector<std::string>& variables, std::vector<LVariableInfo>& fvis)
{


  for (llvm::Function::arg_iterator I = _Function->arg_begin(), E = _Function->arg_end();
  I != E; ++I)
  {
...
void LFunction::SetAddresses()
{
  ...
  std::map<std::string, llvm::Type*>::iterator it = _References.begin(), ie = _References.end();
  for( ; it!=ie; ++it)  
  {	
      //FIXME AKO NEKO PREKORACI TACNO SIZE*1000 ONDA DOLAZI DO GRESKE
      unsigned size = GetParentModule()->GetTargetData()->getTypeAllocSize(it->second)*1000;
...
void LFunction::GetAddresses(unsigned& current, std::vector<argo::Expression>& eqs) const
{
  ...
  for( ; it!=ie; ++it)  
  {
   //TODO zbog free dodati proveru da li je second nula
    ...
    {
    argo::Expression e1 = argo::Expression::Equality(ExpVar(it->first, fpointer_type, false), ExpNum1(current, fpointer_type));
    argo::Expression e2 = argo::Expression::Equality(ExpVar(it->first, fpointer_type, false), ExpNumZeroPtr);
    ...
    else if(isAddress(it->first)) e = argo::Expression::Equality(ExpVar(it->first, fpointer_type, false), ExpNum1(current, fpointer_type));
    else e = argo::Expression::Equality(ExpAddress(_FunctionName, it->first), ExpNum1(current, fpointer_type));
...
{
  if(GetParentModule()->GlobalReferences().find(gv)!=GetParentModule()->GlobalReferences().end()) return; 
  ...
  GetParentModule()->_Addresses.insert(std::pair<std::string, unsigned>(e.GetName(), size));
  GetParentModule()->GlobalMemoryOffset() = GetParentModule()->GetGlobalMemoryOffset()+size;
}

void LFunction::AddReferences(llvm::Instruction* instr, unsigned op)
{

    llvm::AllocaInst* ai = llvm::dyn_cast<llvm::AllocaInst>(instr->getOperand(op));
    llvm::GlobalVariable* gv = llvm::dyn_cast<llvm::GlobalVariable>(instr->getOperand(op));
...
//FIXME glupo je da funckija postavlja global constraints - to bi trebao da radi modul!
void LFunction::SetReferences()
{
  ...
  for (llvm::inst_iterator II = inst_begin(_Function), E = inst_end(_Function); II != E; ++II) 
  {
    ...
    {     
//        llvm::AllocationInst* ai = llvm::dyn_cast<llvm::AllocationInst>(instr->getOperand(0));
        llvm::AllocaInst* ai = llvm::dyn_cast<llvm::AllocaInst>(instr->getOperand(0));
        if(ai)
        {
...
//fixme ovde se nekada doda referenca koja nije potrebna, nije mi jasno zasto se ovo uopste dodaje
    ...
    if(instr->getOpcode() == llvm::Instruction::ICmp ||instr->getOpcode() == llvm::Instruction::FCmp)
    {   //bojim se da ovde moze da bude nesto pogresno sa tipovima, npr da se dobije tip pokazivaca a da treba tip onoga na sta pokazivac pokazuje    
        AddReferences(instr, 0);
    ...
    if((instr->getOpcode() == llvm::Instruction::Call) || (instr->getOpcode() == llvm::Instruction::Invoke))
    {
      ...
      if (f && f->isDeclaration()) 
      {
          if(f->getIntrinsicID() == llvm::Intrinsic::not_intrinsic)
            ...
            _Addresses.insert(std::pair<std::string, unsigned>( MALLOC+GetOperandName(instr), 100000 ));
            }
      }
...
void LFunction::AddFixedAddresses(LFunction* ff)
{
  ...
  std::map<std::string, unsigned>::iterator it = Addresses().begin(), ite = Addresses().end();
   std::string name;
  for(; it!=ite; ++it)
  {
   if(isGlobalAddress(it->first)) name = it->first;
   else if(isMalloc(it->first)) name = AddContext(it->first, GetContext(), GetFunctionName());
   else if(isAddress(it->first)) name = AddContext(it->first, GetContext(), GetFunctionName());
   else name = AddContext(AddressName(ff_function_name, it->first), GetContext(), GetFunctionName());
...
void LFunction::SetStoresInBlocks()
{
  ...
  AddFunctionArguments(variables, fvis); //ovime se gubi efikasnost, argumenti se obicno pojavljuju samo u prvom bloku, glupo je vuci ih svuda
...
_Function->getType()->print(rso);
    ...
    f << "<hr>";
    f << "<font face = \"Courier New\" size = \"2\"> function name: <tt>" 
      << s << "</tt><br> type: <tt>" 
      //<< *_Function->getType()
      << rso.str() 
      << "</tt></font></h4>" << std::endl;
...
void LFunction::PrintHTMLHeader(const std::string& sFilename) const
{
  ...
  if(!f.is_open()) {perror(((std::string)"LFunction::PrintHTMLHeader Could not open html file "+ sFilename)); return;}
  PrintHTMLHeader(f);
  f.close();
...
if(it!=_LoopMax.end()) return it->second;
return 0;
}
...
void LFunction::AddLoopMax(const std::pair<unsigned, unsigned> &loop)
{
    ...
    if(it!=_LoopMax.end())
        {if(it->second<raz) it->second = raz;}
    else _LoopMax.insert(std::pair<unsigned, unsigned>(petlja, raz));
...
{
//ovde je problem sa solverom koji bi trebalo pozivati malo lokalno malo globalno
CalculateParallel();
}*/
}
...
{
  if(!_PostconditionIsSet) SetPostcondition();
