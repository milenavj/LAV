...
void LConstraints::AddAddressConstraint(long long& currentAddress, const argo::Expression& e, int inc)
{
...
void LConstraints::AddConstraint(const argo::Expression& e, unsigned left, unsigned right)
{
  if(CheckPointers)
  {
    argo::IntType iType = fint_type;
    argo::Expression leftside = argo::Expression::Equality(ExpLeft(e), ExpNum1(left, iType));
    argo::Expression rightside = argo::Expression::Equality(ExpRight(e), ExpNum1(right, iType));
    Add(leftside);
    Add(rightside);
  }
}
