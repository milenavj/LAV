...
llvm::cl::opt<std::string> _OutputFolder(
    "output-folder",
...
llvm::cl::opt<std::string> directory(
    "directory",
    llvm::cl::desc(
        "testLAV --- files from this directory will be tested/generated"),
    llvm::cl::init("-"));

llvm::cl::opt<std::string> testFile(
    "test-file",
    llvm::cl::desc("testLAV --- file that contains LAV options that will be "
                   "used for testing/generating (default = \"test\")"),
    llvm::cl::init("test"));

llvm::cl::opt<std::string>
    lavOptions("lav-options",
               llvm::cl::desc("testLAV --- additional Lav options that will be "
                              "applied to default ones  (default = \"\")"),
               llvm::cl::init(""));

llvm::cl::opt<std::string>
    logFile("log-file", llvm::cl::desc("testLAV --- file that will contain LAV "
                                       "results (default = \"results.log\")"),
...
llvm::cl::opt<std::string> modelFile(
    "model-file",
...
llvm::cl::opt<bool>
    sameOptions("same-options",
                llvm::cl::desc("testLAV --- same options (given in a testfile) "
                               "for all programs (default = false)"),
                llvm::cl::init(false));
}

namespace lav {

std::string OutputFolder() { return _OutputFolder; }

bool TestMode() { return testMode; }

bool SameOptions() { return sameOptions; }

bool Model() { return model; }

std::string ModelFile() { return modelFile; }

std::string LogFile() { return logFile; }

std::string Directory() { return directory; }

std::string TestFile() { return testFile; }

std::string LavOptions() { return lavOptions; }
...
void AddDirectoryToFileName(const char *directoryName, const char *fileName,
                            char *file);
void CallLlvmGcc(const char *directoryName, char *obj, char *fileName);
void MakeExpectedOutputName(char *outputName, char *fileName,
...
void CallLav(char *command, char *objname, char *log_file, const char *s,
             const char *additionalOptions, int &verified, int &bugs, char *ver,
             char *bug, char *fileName);
bool CheckFiles(FILE *ExpectedOutput, FILE *LavOutput, const char *filename,
                const char *log_file);
FILE *OpenForReading(const char *filename, const char *file,
                     const char *log_file, const char *cf);
bool CheckModel(const char *directoryName, char *objname, char *filename,
                const char *log_file, const char *modelFile);
bool Check(const char *directoryName, char *objname, char *filename,
           const char *log_file);
...
void osisaj(const char *fileName, char *file1, const char *directoryName,
            char *dir1);
...
void ReadDirectoryItems(DIR *dir, const char *dname, FILE *fout) {
  ...
  while (1) {
    ...
    if (!strstr(dent->d_name, ".c"))
      continue;
    if (strstr(dent->d_name, ".c~"))
      continue;
    if (strstr(dent->d_name, ".cf"))
      continue;
...
FILE *OpenLogFile(const char *log_file) {
  ...
  if (flog == NULL) {
    exit_error((std::string) "Could not open log file " + (std::string)
               log_file);
  }
  return flog;
}
  ...
  while (1) {
    ...
    if (s1 == NULL || s2 == NULL || mystrcmp(str1, str2)) {
      if (s1 == NULL) {
        ...
        perror(
            (std::string) "CheckFiles 3: File name " + filename +
                "\nExpectedOutput\n" + str1 + " \nLavOutput\n " + str2 + "\n",
            log_file);
      }
...
FILE *OpenForReading(const char *filename, const char *file,
                     const char *log_file, const char *callingFunction) {
  FILE *f = fopen(filename, "r");
  if (f == NULL) {
    fprintf(stderr, "Error opening file: %s\n", strerror(errno));
    perror(std::string("Could not open file: ") + std::string(filename) +
               " from function " + callingFunction,
           log_file);
  }
  return f;
}
...
bool CheckModel(const char *directoryName, char *objname, char *filename,
  ...
  while (fgets(s, MAX_BUF, modeli) != NULL) {
    int len = strlen(s);
    if (len > 0)
      s[len - 1] = '\0'; //izbrise znak za kraj
    if (!strcmp(s, filename))
      break;
  }

  if (fgets(s, MAX_BUF, modeli) == NULL) {
    perror((std::string) "Could not read model file name for " + filename +
           " in " + modelFile + " file \n");
    fclose(modeli);
    return false;
  }
  ...
  int len = strlen(s);
  if (len > 0)
    s[len - 1] = '\0'; //izbrise znak za novi red
...
DIR *OpenDir(const char *directoryName) {
  ...
  if (dir == NULL) {
    exit_error(std::string("Could not open directory ") + (std::string)
               directoryName);
  }
  return dir;
}
...
FILE *MakeProgramsList(const char *directoryName) {
  ...
  if (fout == NULL) {
    exit_error("Nije uspleo otvaranje ProgramList datoteke za pisanje");
  }
  printf("\n-----------------------------------------------------------\n");
  printf("%s\n", directoryName);
  ...
  if (fout == NULL) {
    exit_error("Nije uspleo otvaranje ProgramList datoteke za citanje");
  }
...
bool GetFileName(char *cname, FILE *test) {
  //ako je ucitana komanda ispravna, onda se ucitava ime c fajla, ono je u
  //sledecem redu
...
void osisaj(const char *fileName, char *file1, const char *directoryName,
  ...
  if (i > 0) {
    i++;
    for (m = 0, k = i; k <= (signed)(strlen(fileName) - 1); k++, m++)
      file1[m] = fileName[k];
    file1[m] = '\0';
    if (strlen(dir1) > 0)
...
void Test(const char *directoryName, const char *testFileName,
  ...
  if (sameOptions) {
    if (test == NULL)
      ...
      if (!GetCommand(command1, test)) {
        exit_error("Greska u test file-u");
      } else {
        if (!IsCommand(command1)) {
          exit_error("Greska u test file-u");
        }
  ...
  for (;;) {
    ...
    if (test == NULL) //znaci da nemamo testfile nego obilazmo direktorijum
        {
      if (fgets(fileName, MAX_BUF, fout) == NULL)
        break;
      ...
      osisaj(fileName, file1, "", dir1);
        ...
        {
      if (!GetCommand(command, test))
        break;
      if (!IsCommand(command))
        continue;
      if (!GetFileName(cname, test))
        break;
      fileName = cname;
    ...
    CallLav(command, objname, log_file, job, lavOptions, verified, bugs,
            ver_file, bug_file, fileName);
