...
ThreadPool::ThreadPool(ThreadPool &&t) : m_tasks { std::move(t.m_tasks) }
, m_threads { std::move(t.m_threads) }
, m_active { t.m_active }
{}
...
bool ThreadPool::IsActive() { return m_active; }

// Making working lambda for every worker
void ThreadPool::InitWorkerThreads() {
  // Function for every thread
  auto f = [ = ](std::shared_ptr<FixedQueue<std::function<int()> > > queuePtr,
  ...
  // Bind tasks
  auto queuePtr =
      std::make_shared<FixedQueue<std::function<int()> > >(std::move(m_tasks));
...
void ThreadPool::Start() {
  ...
  m_cancel_event = Event::Create();
    ...
    // Create control thread and force it to wait for events of working threads,
    // and wait for cancel event
  m_control_thread = std::thread([this]() {
    ...
    while (num_finished_tasks != m_num_tasks && !cancel) {
      ...
      for (auto idx : vidxs) {
        ...
        // cout << "NUM FINISHED " << num_finished_tasks << "  signal " <<
        // value << endl;
        // Event::Sigval value = events[idx]->Value();
