...
CancelableThread::CancelableThread(Callable f)
    :m_worker{[=](){ AllowCancel(); f();}}, m_finished{false}
{}


CancelableThread::CancelableThread(CancelableThread &&ct)
    :m_worker{move(ct.m_worker)},m_finished{ct.m_finished}
{}
...
void CancelableThread::Cancel()
{
    ...
    if(!m_finished)
    {
        ...
        if(tid)
        {
            // We cancel thread or ignore error if we cannot find it cause it already finished
            int pthreadErrno;
            if((pthreadErrno = pthread_cancel(tid)) != 0 && pthreadErrno != ESRCH)
                throw runtime_error{string{"Thread cancelation failed. Error: "} + ::strerror(pthreadErrno)};
...
bool CancelableThread::IsJoinable() const
{
...
void CancelableThread::AllowCancel()
{
    ...
    if(pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, nullptr) != 0)
        throw runtime_error{string{"Enabling thread cancelation failed. Error: "} + ::strerror(errno)};
    // Set thread to be cancelable at any time
    if(pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, nullptr) != 0)
        throw runtime_error{string{"Setting thread to be cancelable at any time failed. Error: "} + ::strerror(errno)};
