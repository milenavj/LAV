...
Event::Event()
	:m_fd{::eventfd(0,EFD_SEMAPHORE)}, m_ready{false}, m_unsat{false},  m_finished{false}
{
	if(m_fd == InvalidEvent)
		throw std::runtime_error(std::string{"Threads::Greska u pravljenju Event objekta."}+ ::strerror(errno));
}

Event::Event(Event &&e)
	:m_fd{e.m_fd}, m_ready{e.m_ready}, m_unsat{e.m_unsat}, m_finished{e.m_finished}
{
	e.m_fd = InvalidEvent;
	e.m_ready = false;
}
...
void Event::Signal(Sigval value)
{
	...
	if(::write(m_fd, &buff, sizeof(uint64_t)) == -1)
		throw std::runtime_error{ std::string{"Threads::Neuspelo slanje signala u klasi Event. --- "} + ::strerror(errno)};
}
...
Event::Sigval Event::Value(bool block)
{
	...
	if(!m_ready && !block)
		throw std::runtime_error{ std::string{"Threads::Event nije spreman a pokusalo se sa citanjem."}};

	// Citamo vrednost iz fd-a tj. smanjujemo njegov brojac
	uint64_t buff;
	if(::read(m_fd, &buff, sizeof(uint64_t)) == -1)
		throw std::runtime_error{ std::string{"Threads::Neuspelo citanje vrednosti signala. --- "} + ::strerror(errno)};
...
std::vector<size_t> Event::WaitForEvents(std::vector<Event::Pointer> &events, const std::chrono::milliseconds &waitMs, bool blosk)
{
	...
	if(eventsOccured == -1)
		throw std::runtime_error { std::string{"Threads::Greska prilikom cekanja na eventove. --- "} + ::strerror(errno)};
	...
	if(eventsOccured)
	{
		...
		for(auto it = pevents.cbegin(); it != pevents.end(); ++it)
		{
			...
			{
				throw std::runtime_error{ std::string{"Threads::Greska prilikom cekanja na event: "} + std::to_string(idx) + " --- " + ::strerror(errno) };
