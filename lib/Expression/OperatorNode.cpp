...
ExpressionNode *
OperatorNode::SetOperands(const std::vector<Expression> &operands) const {
  return new OperatorNode(_type, _name, operands);
}

size_t OperatorNode::GetArity() const { return _operands.size(); }
...
size_t OperatorNode::hashCode() const {
  ...
  for (; i < iend; i++)
    hc = hc * PRIME_HASH +
         long_int_hash_func(reinterpret_cast<unsigned long>(i->_expression));
  return hc;
}
...
int OperatorNode::compare(const ExpressionNode &e) const {
  ...
  unsigned n1 = operands_1.size(), n2 = operands_2.size();
...
ExpressionNode *ANDNode::Simplify(const std::vector<Expression> &operands) {
  ...
  for (it = operands.begin(); it != iend; ++it) {
    ...
    if (it->IsAND()) {
      std::vector<Expression>::const_iterator j, jend = it->end();
      for (j = it->begin(); j != jend; ++j)
        //Add arguments that don't already exist
        if (find(new_operands.begin(), new_operands.end(), *j) ==
            new_operands.end())
          new_operands.push_back(*j);
    } else
        //Add arguments that don't already exist
        if (find(new_operands.begin(), new_operands.end(), *it) ==
            new_operands.end())
      new_operands.push_back(*it);
  }
  ...
  for (i = operands.begin(); i != iend; i++) {
    ...
    if (i->IsOR()) {
      ...
      for (j = i->begin(); j != jend; j++)
        ...
        //Add arguments that don't already exist
        if (find(new_operands.begin(), new_operands.end(), *i) ==
            new_operands.end())
