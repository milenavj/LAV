...
FormulaVector::FormulaVector(size_t width) {
  ...
  for (unsigned int i = 0; i < size; i++) {
    Formula *f = FormulaFactory::Instance()->Get(
        new FormulaVar(++m_iPropositionalVarCounter));
    Formula::NEW++;
    bitArray[i] = f;
    f->IncRefCount();
  }
...
Formula *FormulaVector::operator[](unsigned int i) const { return bitArray[i]; }
...
FormulaVector FormulaVector::ite(const FormulaVector &fv1,
                                 const FormulaVector &fv2) const {
  assert(size == 1);
