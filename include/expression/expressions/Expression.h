...
/** 
* \brief Central class used for representation of First Order Logic Expressions (Terms and Formulae). 
*
* Expression class implements the reference counted smart pointer functionality to ExpressionNodes.
* All common subexpressions of an expression are shared. All existing ExpressionNodes are registered 
* in an ExpressionFactory. When a new Expression is created, ExpressionFactory is consulted. If a
* desired ExpressionNode already exists in the system, newly created Expression just points to it.
* \nosubgrouping
*/
class Expression {	
 ...
 private:
    ...
    /** 
     * Unique representative for the given ExpressionNode, given by the ExpressionFactory, is attached to the Expression.  
     */
    void AttachExpressionNode(ExpressionNode* e);

    /**
     *	Expression node is detached from the Expression. It is automatically garbage-collected if 
     ...
     *  Static functions and operators used to create First Order Logic Expressions (Terms and Formulae)
     */	
    //@{
 public:
    static Expression Variable(const std::string name, IntType t = noInt, bool r = true); 
    ...
    static Expression IfThenElse(const Expression& formula, const Expression& arg1, const Expression& arg2);
    static Expression Function(const std::string name, const std::vector<Expression>& arguments, IntType intType = noInt);
    static Expression Function(const std::string name, const Expression& arg1, IntType intType = noInt);
    static Expression Function(const std::string name, const Expression& arg1, const Expression& arg2, IntType intType = noInt);
    static Expression Function(const std::string name, const Expression& arg1, const Expression& arg2, const Expression& arg3, IntType intType = noInt);
    static Expression Function(const std::string name, const Expression& arg1, const Expression& arg2, const Expression& arg3, const Expression& arg4, IntType intType = noInt);
    static Expression Predicate(const std::string name, const std::vector<Expression>& arguments);
    static Expression Predicate(const std::string name, const Expression& arg1);
    static Expression Predicate(const std::string name, const Expression& arg1, const Expression& arg2);
    static Expression Predicate(const std::string name, const Expression& arg1, const Expression& arg2, const Expression& arg3);
    ...
    static Expression array(const Expression& name, const Expression& indexW, const Expression& valueW);
    bool isArray() const;
    ...
    /**
     *   Prints the Expression to a given output stream using given OutputFormater.
     */
    ...
    /**
     *	Gets the expression's name i.e. its head. E.g. the function symbol, the predicate symbol, connective symbol, etc.
     */
    ...
    /**
     *	Queries the sort of the expression. In a multisorted logic framework, each Expression is given a unique sort.
     *  It is determined from the signature of each function and/or predicate symbol and the sorts of its subexpressions.\
     *  E.g. If x, y : REAL and rel : REAL, REAL -> LOGICAL, than the sort of the expression rel(x, y) is LOGICAL.
     */
    SORT GetSort () const {
	...
	return GetType() == EXPR_VARIABLE;	
    }

    /**
     *	Checks if Expression is a propositional variable
     */
    bool IsFormulaVariable() const {
	return GetType() == EXPR_FORMULA_VARIABLE; 
    }

    /**
     *	Checks if Expression is a metavariable. Metavariables are used for unification.
     */
    bool IsMetaVariable() const {
	return GetType() == EXPR_METAVARIABLE;
    }
	...
	return GetType() == EXPR_INT_NUMERAL ||
	    GetType() == EXPR_RATIONAL_NUMERAL;	
    }
	...
	return GetType() == EXPR_DISEQUALITY;	
    }
 ...
 public:
    ...
    /**
     *	Returns an operands_iterator that points to the first of the expression's operands
     */
    ...
    /**
     *	Returns an operands_iterator that points to the first positon pass the last of the expression's operands
     */
	...
	return _expression->hasOperands();
    }

    /**
     *	Gets a const vector that contains expression's operands. If there are no operands, empty vector is returned.
     */
...
//	assert(hasOperands() && 0<=num && num<_expression->GetArity());
    ...
    /**
     *	Gives the subexpressions_iterator that points to the first subexpression of the Expression
     */
    ...
    /**
     *	Gives the subexpressions_iterator that points to the first position after the last subexpression of the Expression
     */
    ...
    /**
     *	Checks if a given expression occurs as a subexpression of a given expression
     */
    bool ContainsSubexpression(const Expression& subexpression) const;
    bool ContainsSubexpression(const Expression& subexpression, Position& position) const;

    /**
     *	Performs the substitution of all ocurrences of a given expression (in the expression)
    ...
    /**
     *	A complexity measure for the expression. 1 for atoms, and (sum of operand sizes+1) for compound expressions
     */
    int GetSize() const	{
	return _expression->GetSize();
    }
    ...
    void ResetUnionFind() {
	_expression->ResetUnionFindParrent();
    }
