...
//      return Z3Instance::instance().getIntAssignment(_expr);
    ...
    LAExpressionImpZ3* unsignedSymbolic(const std::string& name, size_t width) const {
...
//FIXME ovo se u yices-u razlikuje - nat i int????
    LAExpressionImpZ3* signedSymbolic(const std::string& name, size_t width) const {
      Z3_sort sort = Z3_mk_int_sort(getSolver());
      Z3_symbol   s  = Z3_mk_string_symbol(getSolver(), name.c_str());
      SOLVER_EXPR_TYPE exp = Z3_mk_const(getSolver(), s, sort);
      return new LAExpressionImpZ3(exp);
    ...
    virtual ExpressionImp* uninterpretedFunction(const Function& fun, const std::vector<const ExpressionImp*>& args) {
      ...
      for (i = args.begin(), k = 0; i != args.end(); i++, k++) {
        ...
        const LAExpressionImpZ3* y = 
          llvm::dyn_cast<LAExpressionImpZ3>(*i);
        if (u != 0) {
//FIXME moze i signed
...
//FIXME un-signed
	...
	Z3_mk_add(getSolver(), 2, args);
      return new LAExpressionImpZ3(exp);
    }

    virtual ExpressionImp* addSymbolic(const ExpressionImpSymbolic* const e) const {
      const LAExpressionImpZ3* const bexpr = 
	llvm::dyn_cast<LAExpressionImpZ3>(e);
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE args[2];
      args[0] = this->_expr;
      args[1] = bexpr->_expr;
...
//FIXME signed
	...
	llvm::dyn_cast<LAExpressionImpZ3>(e);
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
	Z3_mk_lt(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpZ3(exp);
    }
...
//FIXME 
      ...
      const LAExpressionImpZ3* const s =
        llvm::dyn_cast<LAExpressionImpZ3>(e);
      assert(s != 0);
      return s->_expr;
    }
    ...
    virtual ExpressionImp* store(const ExpressionImp* const array, const ExpressionImp* const index, const ExpressionImp* const value) const {
     const LAExpressionImpZ3* sarray =
        llvm::dyn_cast<LAExpressionImpZ3>(array);
  ...
  return new LAExpressionImpZ3(exp);//, sarray->_width, Z3_BITVECTOR);
    ...
    virtual ExpressionImp* uleSymbolic(const ExpressionImpSymbolic* const e) const {
      ...
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
        Z3_mk_le(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpZ3(exp);
    }
	...
	llvm::dyn_cast<LAExpressionImpZ3>(e);
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
	Z3_mk_gt(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpZ3(exp);
    }
    ...
    virtual ExpressionImp* ugeSymbolic(const ExpressionImpSymbolic* const e) const {
      ...
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
        Z3_mk_ge(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpZ3(exp);
    }
    ...
    virtual ExpressionImp* neqGround(const ExpressionImpGroundInteger* const e) const {
      SOLVER_EXPR_TYPE exp = 
        Z3_mk_not(getSolver(), Z3_mk_eq(getSolver(), solverSignedExprFromGround(e), this->_expr));
      return new LAExpressionImpZ3(exp);
    }
    ...
    virtual ExpressionImp* ite_(ExpressionImp* thenExpr,
      ...
      LAExpressionImpZ3*  bthenExpr = 
	llvm::dyn_cast<LAExpressionImpZ3>(thenExpr);
      LAExpressionImpZ3*  belseExpr =
	llvm::dyn_cast<LAExpressionImpZ3>(elseExpr);
  ...
  private:
    ...
    SOLVER_EXPR_TYPE solverUnsignedExprFromGround(const ExpressionImpGround* const e) const {
  ...
  public:
    LAExpressionFactoryZ3() 
      : ExpressionFactory(new LAExpressionImpZ3()) {
    }
  };
