...
namespace UrsaMajor {
  ...
  public:
    ...
    static Expression uninterpretedFunction(const Function& f, const std::vector<Expression>& args);
    ...
    static Expression store(const Expression& array, const Expression& index, const Expression& value);
...
//    Expression operator<  (const Expression& expr) const;
    ...
    ExpressionImp* getImp() const
    {return _imp.get();}
//    static std::map<std::string, std::tr1::shared_ptr<ExpressionImp> > _symbolTable;
  private:
  ...
  public:
    ...
    virtual ExpressionImp* signedSymbolic(const std::string& name, size_t width) const = 0;
    virtual ExpressionImp* unsignedSymbolic(const std::string& name, size_t width) const = 0;
    ...
    virtual ExpressionImp* integerGround(unsigned short x, size_t width) const = 0;
    ...
    virtual ExpressionImp* addGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* addSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* subtract(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* subtractGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* subtractSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* subtractFromGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* subtractFromSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* mult(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* multGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* multSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* udivimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* udivGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* udivSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* udivFromGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* udivFromSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* sdivimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* sdivGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sdivSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* sdivFromGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sdivFromSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* uremimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* uremGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* uremSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* uremFromGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* uremFromSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* sremimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* sremGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sremSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* sremFromGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sremFromSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* bitXor(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* bitXorGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* bitXorSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* bitAnd(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* bitAndGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* bitAndSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* logicalAnd(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* logicalAndGround(const ExpressionImpGroundBoolean* const e) const = 0;
    virtual ExpressionImp* logicalAndSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* bitOr(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* bitOrGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* bitOrSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* logicalOr(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* logicalOrGround(const ExpressionImpGroundBoolean* const e) const = 0;
    virtual ExpressionImp* logicalOrSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    ...
    virtual ExpressionImp* shiftLForGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* shiftLForSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* shiftLGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* shiftLSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* aShiftRimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* aShiftRForGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* aShiftRForSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* aShiftRGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* aShiftRSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    
    virtual ExpressionImp* lShiftRimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* lShiftRForGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* lShiftRForSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    virtual ExpressionImp* lShiftRGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* lShiftRSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* ultimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* ultGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* ultSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* ugtimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* ugtGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* ugtSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* uleimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* uleGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* uleSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* ugeimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* ugeGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* ugeSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* sltimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* sltGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sltSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* sgtimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* sgtGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sgtSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* sleimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* sleGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sleSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* sgeimp(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* sgeGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* sgeSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* eq(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* eqGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* eqGround(const ExpressionImpGroundBoolean* const e) const = 0;
    virtual ExpressionImp* eqSymbolic(const ExpressionImpSymbolic* const e) const = 0;

    virtual ExpressionImp* neq(const ExpressionImp* const e) const = 0;
    virtual ExpressionImp* neqGround(const ExpressionImpGroundInteger* const e) const = 0;
    virtual ExpressionImp* neqGround(const ExpressionImpGroundBoolean* const e) const = 0;
    virtual ExpressionImp* neqSymbolic(const ExpressionImpSymbolic* const e) const = 0;
    ...
    virtual ExpressionImp* uninterpretedFunction(const Function& fun, const std::vector<const ExpressionImp*>& args) = 0;
    virtual ExpressionImp* array(const std::string& name, size_t domain, size_t range) = 0;

    virtual ExpressionImp* select(const ExpressionImp* const array, const ExpressionImp* const index) const = 0;
    virtual ExpressionImp* store(const ExpressionImp* const array, const ExpressionImp* const index, const ExpressionImp* const value) const = 0; 

    // TODO: FIXME const correctness? 
    virtual ExpressionImp* ite(ExpressionImp* thenExpr,
			       ExpressionImp* elseExpr, size_t width) const = 0;
  ...
  public:
    ...
    virtual bool checkSat() {
      throw "ExpressionImpGround::checkSat() N/A";
    }
    ...
    virtual void reset() {
      throw "ExpressionImpGround::reset() N/A";
    }
    ...
    virtual ExpressionImp* ite(ExpressionImp* thenExpr,
			       ExpressionImp* elseExpr, size_t width) const {
  ...
  public:
    ...
    virtual bool isGroundBoolean() const {
      return true; 
    }
...
//      return "";
  ...
  public:
    ...
    explicit ExpressionImpGroundInteger(unsigned n, size_t width) 
      : ExpressionImpGround(EK_ExpressionImpGroundInteger), _n(BitVector::fromInteger(n, width)) {
    }
    ...
    virtual bool isGroundInteger() const {
      return true; 
    }
    ...
    virtual bool getBooleanValue() const {
      throw "getBooleanValue N/A";
    }

    virtual unsigned long getUnsignedValue() const {
      return _n.toUnsigned();
    }
...
//      return new ExpressionImpGroundInteger(this->_n % e->_n);
    ...
    virtual ExpressionImp* aShiftRGround(const ExpressionImpGroundInteger* const e) const {
//      return new ExpressionImpGroundInteger(e->_n >> this->_n.toUnsigned());
      return new ExpressionImpGroundInteger((e->_n).aShiftRight(this->_n.toUnsigned()));
    }

    virtual ExpressionImp* aShiftRForGround(const ExpressionImpGroundInteger* const e) const {
//      return new ExpressionImpGroundInteger(this->_n >> e->_n.toUnsigned());
      return new ExpressionImpGroundInteger((this->_n).aShiftRight(e->_n.toUnsigned()));
    }
    ...
    virtual ExpressionImp* lShiftRGround(const ExpressionImpGroundInteger* const e) const {
//      return new ExpressionImpGroundInteger(e->_n >> this->_n.toUnsigned());
      return new ExpressionImpGroundInteger((e->_n).lShiftRight(this->_n.toUnsigned()));
    }

    virtual ExpressionImp* lShiftRForGround(const ExpressionImpGroundInteger* const e) const {
//      return new ExpressionImpGroundInteger(this->_n >> e->_n.toUnsigned());
      return new ExpressionImpGroundInteger((this->_n).lShiftRight(e->_n.toUnsigned()));
    }
...
//FIXME - na drugo mesto prebaciti
  virtual ExpressionImp* uninterpretedFunction(const Function& fun, const std::vector<const ExpressionImp*>& args) {
      std::cout << "ExpressionImpSymbolic --- sta cu ovde " << fun.getName() << std::endl;
  ...
  private:
    ...
    virtual ExpressionImp* ite(ExpressionImp* thenExpr,
			       ExpressionImp* elseExpr, size_t width) const {
      if(thenExpr==NULL || elseExpr==NULL) {std::cerr << "thenExpr==NULL || elseExpr==NULL" << std::endl; return NULL;}
      ...
      return ite_(gthen == 0 ? thenExpr : integerGround(gthen->getUnsignedValue(), width),
		  gelse == 0 ? elseExpr : integerGround(gelse->getUnsignedValue(), width));
    }
...
//    }
  ...
  private:
    ExpressionImp* _expr;
  public:
    explicit ExpressionFactory(ExpressionImp* expr) 
      : _expr(expr) {
    }
    ...
    virtual Expression array(const std::string& name, size_t domain, size_t range);

    virtual Expression select(const Expression& array, const Expression& index) const;

    virtual Expression store(const Expression& array, const Expression& index, const Expression& value) const;
