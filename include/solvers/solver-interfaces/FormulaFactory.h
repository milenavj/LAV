...
//		formulaFactory = 0;
  ...
  bool to_cnf(FormulaVector constraint, vector<vector<int> >& conj, vector<unsigned int>& MappedVarId, unsigned BasicVarCount, unsigned* varCount);

private:
  void SetDeepArgsNeeded(Formula* f, FORMULA_TYPE type, vector<bool>& neededFormulas);
