...
namespace UrsaMajor {
  ...
  public:
    ...
    LAExpressionImpYices* unsignedSymbolic(const std::string& name, size_t width) const {
    ...
    LAExpressionImpYices* signedSymbolic(const std::string& name, size_t width) const {
    ...
    LAExpressionImpYices* integerGround(unsigned short int x, size_t width) const {
      SOLVER_EXPR_TYPE exp = yices_mk_num(getSolver(), x);
      return new LAExpressionImpYices(exp);
    }
...
std::map<std::string, yices_expr>& _uf_registry = YicesInstance::instance()._uf_registry;
      ...
      for (i = args.begin(), k = 0; i != args.end(); i++, k++) {
        ...
        const LAExpressionImpYices* y = 
          llvm::dyn_cast<LAExpressionImpYices>(*i);
        if (u != 0) {
	...
	llvm::dyn_cast<LAExpressionImpYices>(e);
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
	yices_mk_lt(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpYices(exp);
    }
    ...
    virtual ExpressionImp* uleSymbolic(const ExpressionImpSymbolic* const e) const {
      ...
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
        yices_mk_le(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpYices(exp);
    }
//FIXME da li ovde treba slt-sle-sgt-sge a yices to nema ili nije ni vazno
	...
	llvm::dyn_cast<LAExpressionImpYices>(e);
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
	yices_mk_gt(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpYices(exp);
    }
    ...
    virtual ExpressionImp* ugeSymbolic(const ExpressionImpSymbolic* const e) const {
      ...
      assert(bexpr != 0);
      SOLVER_EXPR_TYPE exp = 
        yices_mk_ge(getSolver(), this->_expr, bexpr->_expr);
      return new LAExpressionImpYices(exp);
    }
...
//    static SOLVER_TYPE _ctx;
  ...
  public:
    LAExpressionFactoryYices() 
      : ExpressionFactory(new LAExpressionImpYices()) {
    }
  };
