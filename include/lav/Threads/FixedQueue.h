...
namespace Threads
{
    ...
    public:
        FixedQueue(const std::vector<T> &data)
            :m_data{data}, m_idxPtr{new std::atomic_uint{0}}
        {}

        FixedQueue(FixedQueue &&fq)
            :m_data{std::move(fq.m_data)}, m_idxPtr{std::move(fq.m_idxPtr)}
        {}
