...
#define TRANSFORMATION "transformation_"
#define ACTIVEBLOCK "active_block_"
#define MEMORY "MEMORY"
#define GLOBAL_MEMORY "GLOBAL_MEMORY"
...
#define ExpArray(name) aExp::array(ExpFVar(name), ExpNum(ItoS(GetBitWidth(fpointer_type))), ExpNum(ItoS(GetBitWidth(fint_type))))
#define AddressName(function_name, variable_name) (ADDRESS + function_name +  "_" + variable_name)
...
#define ExpAddress(function_name, variable_name) ExpVar(AddressName(function_name, variable_name), fpointer_type, false)
#define ExpGlobalAddress(variable_name) ExpAddress(std::string(GLOBAL), variable_name)
...
aExp MakeExp(cStr &name, cStr &function, cStr &s, int id, unsigned zero, const llvm::Type* t, int Context = 0, cStr& fname="");
}
