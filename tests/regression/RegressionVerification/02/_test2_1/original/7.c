...
float max_broj(float niz[],
               int n) { /*funkcija sa unos niza argumenti niz i njegova duzina*/
  float max = niz[0];   /*dodelimo maksimumu prvi clan niza*/
  int i;

  for (i = 0; i < n; i++)
    if (niz[i] > max)
      max = niz[i]; /*ukoliko je neki clan niza veci od max, on postaje max*/
...
float aritmeticka_sredina(float niz[], int n) { /*funkcija za arit. sr. niza*/
  ...
  for (i = 0; i < n; i++)
    suma += niz[i];
        /*odredjujemo sumu clanova niza koja nam je potrebna za ar.s*/
...
int main() {
  ...
  if (n <= 0 || n > MAKS_NIZ) { /*ukoliko je duzina niza manja ili jednaka 0,
      greska, ili veca od max 				duzine niza*/
    printf("-1\n");
    return 0;
  }
  ...
  for (i = 0; i < n; i++)
    scanf("%f", &niz[i]); /*petlja za unos clanova niza*/
