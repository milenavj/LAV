...
/**
   The current C API is not yet complete, but the basic Yices functionalities
   can be used.
   Dependent types, quantifiers, lambda expressions are not supported by this
   API right now
...
/**
   ...
   \brief Set the maximum number of conflicts that are allowed in a maxsat
   iteration.

   If the maximum is reached, then "unknown" (i.e., l_undef) is returned by
   maxsat.
 */
void yices_set_max_num_conflicts_in_maxsat_iteration(unsigned n);

/**
   \brief Force Yices to type check expressions when they are asserted (default
   = false).
 */
...
/**
   \brief Display the internal representation of the given logical context on
   <tt>stderr</tt>.
...
/**
   ...
   \sa yices_retract
*/
assertion_id yices_assert_weighted(yices_context ctx, yices_expr expr,
                                   long long w);
...
/**
   ...
   This is similar to #yices_assert_weighted, but the weight is considered to be
   infinite.
...
/**
   ...
   If the context is satisfiable, then #yices_get_model can be used to obtain a
   model.
...
/**
  ...
  If there are no weighted constaints in \c ctx, then this function is the same
  as #yices_check.
...
/**
   ...
   Model \c m can be obtained via #yices_get_model, after a call to
#yices_check,
...
/**
   ...
   - \c l_undef means it was not possible to decide due to an incompleteness.
   If the result is \c l_true, then #yices_get_model can be used to obtain a
model.
...
/**
   ...
   - \c v is not a proper declaration or not the declaration of a numerical
   variable
   - \c v has no value assigned in model m (typically, this means that v does
   not
...
/**
   ...
   \sa yices_get_double_value
*/
int yices_get_arith_value(yices_model m, yices_var_decl d, long *num,
                          long *den);

/**
   \brief Convert the value assigned to variable \c v in model \c m to a
   floating
   point number.
...
/**
   \brief Convert the value assigned to variable \c v in model \c m to a GMP
   rational (\c mpq_t)
   ...
   before \#include <yices_c.h> in your code. If you don't need GMP numbers,
   don't include <gmp.h>.
   - The \c mpq_t object \c value must be initialized first, by calling GMP's
   <tt>mpq_init</tt> function.
...
/**
   ...
   before <tt>\#include <yices_c.h></tt> in your code. If you don't need GMP
   numbers, don't include <gmp.h>.
   - The \c mpz_t object \c value must be initialized first, by calling GMP's
   <tt>mpz_init</tt> function
...
*/
int yices_get_bitvector_value(yices_model m, yices_var_decl d, unsigned n,
                              int bv[]);
...
/**
   ...
   This function is only useful for assertion ids obtained using
   #yices_assert_weighted,
   and #yices_max_sat was used to build the model. That is the only scenario
   where an
...
/**
   ...
   <tt>yices_mk_bool_var(c, n1) == yices_mk_bool_var(c, n2)</tt> when
<tt>strcmp(n1, n2) == 0</tt>.
...
/**
   \brief Return the variable declaration object associated with the given name
   expression.

   \warning \c e must be a name expression created using methods such
   as: #yices_mk_bool_var, #yices_mk_fresh_bool_var, or
...
/**
   \brief Return a name expression (instance) using the given variable
   declaration.
 */
 ...
 */
yices_expr yices_mk_ite(yices_context ctx, yices_expr c, yices_expr t,
                        yices_expr e);
...
/**
   \brief Create an iterator that can be used to traverse the boolean variables
(var_decl objects) in the
...
*/
yices_type yices_mk_tuple_type(yices_context ctx, yices_type *args[],
                               unsigned size);

/**
   \brief Return a new (global) variable declaration. It is an error to create
   two variables
   with the same name.
*/
 ...
 */
yices_expr yices_mk_app(yices_context ctx, yices_expr f, yices_expr args[],
                        unsigned n);
...
/**
  ...
  \warning
  - You must include <gmp.h> before <yices_c.h> for this function to be
  available.
...
*/
yices_expr yices_mk_bv_constant(yices_context ctx, unsigned size,
                                unsigned long value);
...
/**
   ...
   bit \c i of the bitvector is set to 0 if <tt>bv[i] = 0</tt> and to 1 if
   <tt>bv[i] != 0</tt>
*/
yices_expr yices_mk_bv_constant_from_array(yices_context ctx, unsigned size,
                                           int bv[]);
...
*/
yices_expr yices_mk_bv_extract(yices_context ctx, unsigned end, unsigned begin,
                               yices_expr a);
...
*/
yices_expr yices_mk_bv_shift_right0(yices_context ctx, yices_expr a,
                                    unsigned n);

/**
  \brief Right shift by \c n bits, padding with ones
*/
yices_expr yices_mk_bv_shift_right1(yices_context ctx, yices_expr a,
                                    unsigned n);
